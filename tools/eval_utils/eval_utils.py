...
# keys in batch_dict: ['points', 'frame_id', 'calib',
#                      'gt_boxes', 'use_lead_xyz', 'voxels',
#                      'voxel_coords', 'voxel_num_points',
#                      'image_shape', 'batch_size']
# points: [0 x y z intensity(normalize to 0 to 1)]
# frame_id: pop
# calib: pop
# gt_boxes: pop
# use_lead_xyz: true
...
def eval_realtime(cfg, model, batch_dict, logger, dist_test=False, save_to_file=False, result_dir=None):
    ...
    statistics_info(cfg, ret_dict, metric, disp_dict)
    annos = generate_prediction_dicts(
        batch_dict, pred_dicts, class_names,
        None
    )
    det_annos += annos
    ...
    if dist_test:
        rank, world_size = common_utils.get_dist_info()
        det_annos = common_utils.merge_results_dist(
            det_annos, 1, tmpdir=result_dir / 'tmpdir')
        metric = common_utils.merge_results_dist(
            [metric], world_size, tmpdir=result_dir / 'tmpdir')

    sec_per_frame = time.time() - start_time
    logger.info('Generate label finished(sec_per_frame: %.4f second).' %
                sec_per_frame)
